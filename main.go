...
func echoCmd(cmd string) {
	...
	print := strings.TrimPrefix(cmd, "echo ")
	print = strings.TrimSpace(print)

	if strings.HasPrefix(print, `'`) && strings.HasSuffix(print, `'`) {
		print = print[1 : len(print)-1]
		fmt.Println(print)
		return
	}

	if (strings.HasPrefix(print, `"`) && strings.HasSuffix(print, `"`)) || !strings.HasPrefix(print, "'") {
		print = strings.Trim(print, `"`)
		print = strings.TrimSpace(print)

		re := regexp.MustCompile(`\s+`)
		print = re.ReplaceAllString(print, " ")
	}
